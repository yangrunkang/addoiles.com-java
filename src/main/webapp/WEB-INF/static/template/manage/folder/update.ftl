		...
		<div class="row">
			<form id="update_folder_form" method="post"
				...
				<div class="col-lg-12">
					...
					<input type="hidden" class="form-control" name="fatherId" value="${folder.fatherId}">						
					<section class="panel">
						...
						<div class="panel-body">
	                        ...
	                        <div class="form-group">
	                        	...
	                        	<div class="col-sm-10">
	                        		<select class="form-control" name="newfatherId" style="font-size:15px;width:300px;">
	                               		<#list folderAll as folders>
	                               		     <#if folders.owner=="yes">
	                               		     	<option value="${folders.folderId}" <#if folders.folderId==folder.folderId>selected</#if>>
	                               		     	${folders.pathName}
	                               		     	</option>
	                               		     </#if>
	                               		</#list>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$('#file_upload').uploadify({
			'buttonText'  		: 	'请选择文件',
	        'swf'         		: 	'${TEMPLATE_MANAGE_PATH}/assets/uploadify/uploadify.swf',
	        'uploader'    		: 	'${BASE_PATH}/manage/media/upload.json;jsessionid=${JSESSIONID}',
		...
		$('#folderId').change(function(){
			window.location.href = "${BASE_PATH}/manage/media/page.htm?folderId="+$('#folderId').val();
		});
		...
		$('#attachment .js_link').click(function(){
			...
			bootbox.prompt("修改链接，当前链接为："+link, function(result) {
				if (result != null && result != "") {
					$.post("${BASE_PATH}/manage/media/update_link.json",{'attachmentId':attachmentId,'link':result},function(data){
						if(data.result){
							window.location.reload();
						}
					},"json");					
		...
		$('#attachment .js_delete').click(function(){
			...
			bootbox.confirm("是否要删除【"+$(this).attr("name")+"】文件？", function(result) {
				if (result) {
					$.post("${BASE_PATH}/manage/media/delete.json",{'attachmentId':file.attr("attachmentId")},function(data){
						if(data.result){
							window.location.reload();
						}
					},"json");
